...
def bracket_theta_values(
    ...
    kappa_input = as_1d(kappa_input)
    r_input = as_1d(r_input)
    chieff_input = as_1d(chieff_input)
    q_input = as_1d(q_input)
    chi1_input = as_1d(chi1_input)
    chi2_input = as_1d(chi2_input)

    values = np.full(kappa_input.size, np.nan)

    for i in range(kappa_input.size):
        try:
            values[i] = as_1d(
                precession.eval_bracket_theta(
                    float(kappa_input[i]),
                    float(r_input[i]),
                    float(chieff_input[i]),
                    float(q_input[i]),
                    float(chi1_input[i]),
                    float(chi2_input[i]),
                    method="montecarlo",
...
plt.ylabel(r"$\langle\theta_L\rangle$")
