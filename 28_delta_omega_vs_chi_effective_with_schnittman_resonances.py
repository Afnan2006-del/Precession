...
# Calculate the 95th percentile from valid Delta Omega values,
# not from the Boolean finite mask.
valid_absolute_y = np.abs(y[finite])

if valid_absolute_y.size == 0:
    raise RuntimeError(
        "No finite Delta Omega values were produced."
    )

cap = np.percentile(valid_absolute_y, 98)

# Prevent identical y-limits if every valid value is zero.
if not np.isfinite(cap) or cap <= 0.0:
    cap = np.max(valid_absolute_y)

if not np.isfinite(cap) or cap <= 0.0:
    cap = 1.0e-12

# Remove the largest 5% from the displayed Monte Carlo points.
plot_finite = finite & (np.abs(y) <= cap)
